...
but must be manually synced and pushed:

```bash
source .venv/bin/activate

# 1. Sync latest data from S3 → docs/data/
python pgdh_campaign/sync_to_pages.py

# 2. Regenerate the HTML viewer
...
Streamlit app at `dashboard/app.py` reads directly from Lyceum S3 on each visit.
Slower on cold start (~15s to fetch data) but always shows the latest state
without manual syncing. Deployed on Streamlit Community Cloud.
...
- **Jobs** — track Lyceum job status, auto-evaluate on completion (auth required)
- **New Run** — submit design/validation/scoring jobs (auth required)
- **Design Detail** — per-design metrics, sequence, notes, 3D viewer

Read-only pages are public. Write actions (job submission, status updates, notes)
require Google OAuth sign-in.

```bash
# Run locally
cd dashboard && streamlit run app.py

# Or visit the deployed app (Streamlit Community Cloud)
```

**Caching**: The app caches the tracker for 5 min and CIF structures for 10 min
in server memory. Streamlit Community Cloud sleeps after ~15 min of inactivity,
so the first visit after idle always triggers a fresh S3 load.
...
|---------|------|--------|---------|
| `output/boltzgen/` | Raw BoltzGen outputs (CSVs, CIFs) | Lyceum jobs | `evaluate_designs.py` |
| `output/rfdiffusion3/` | Raw RFD3 outputs (JSONs, CIFs) | Lyceum jobs | `evaluate_designs.py` |
| `output/boltz2/` | Boltz-2 refolding predictions | Lyceum jobs | `evaluate_designs.py` |
| `output/ipsae/` | ipSAE binding scores | Lyceum jobs | `evaluate_designs.py` |
| **`designs/index.json`** | **Master ranked index of all designs** | **`evaluate_designs.py` only** | Dashboard, GitHub Pages, skills |
| **`designs/<tool>/<id>/`** | **Per-design: metrics.json + CIFs** | **`evaluate_designs.py` only** | Dashboard, GitHub Pages, skills |
| `tracker/state.json` | Campaign state (jobs, notes, status) | Dashboard, Claude Code | Dashboard, skills |

The `designs/` directory is **read-only** — only `evaluate_designs.py` writes to it.
...
│   ├── evaluate_designs.py        # Unified evaluation pipeline → writes designs/ on S3
│   ├── sync_to_pages.py           # Sync designs/ from S3 → docs/data/ (for GitHub Pages)
│   ├── generate_pages.py          # Generate docs/index.html from docs/data/
...
│   ├── data/                      # Synced from S3 designs/ by sync_to_pages.py
│   │   ├── index.json
│   │   ├── evaluated.json
│   │   ├── unevaluated.json
...
│
├── dashboard/                     # Streamlit web app (reads S3 live)
│   ├── app.py                     # Main app (5 pages)
│   ├── client.py                  # Lyceum API client (with token refresh)

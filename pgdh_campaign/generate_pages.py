...
<div class="tab-quote">&ldquo;The point is not to judge but to understand what molecules are capable of when they are not defined by what we already know.&rdquo;<span class="tq-attr">&mdash; Claude (falsely attributed to Isabelle Stengers)</span></div>
<div class="wf-wrap">
  <div class="wf-title">
...
function iv(i){{if(init[i])return;var e=document.getElementById('viewer_'+i);if(!e||e.offsetParent===null)return;var d=vd[i];if(!d)return;if(!d.cif){{e.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#8b949e;font-size:14px">3D view available for top {MAX_CIF_EMBEDS} designs</div>';init[i]=true;return;}}var v=$3Dmol.createViewer(e,{{backgroundColor:'#0d1117'}});v.addModel(d.cif,'cif');var bc=d.binderChain,tc2=bc==='A'?'B':'A',c=tclr[d.tool]||'#00CED1';v.setStyle({{chain:tc2}},{{cartoon:{{color:'#ff80bf',opacity:0.6}}}});v.setStyle({{chain:bc}},{{cartoon:{{color:c}}}});v.zoomTo();v.render();vs[i]={{viewer:v,binderChain:bc,targetChain:tc2,color:c}};init[i]=true;}}
...
function switchTab(t,pushHash){{var ts=['evaluated','unevaluated','all','table','progress','strategies','target','workflow'];document.querySelectorAll('.tab').forEach(function(e,i){{e.classList.toggle('active',ts[i]===t);}});document.querySelectorAll('.tab-panel').forEach(function(p){{p.classList.remove('active');}});var p=document.getElementById('panel_'+t);if(p)p.classList.add('active');if(t==='table'&&!tableInit){{renderTable();tableInit=true;}}if(t==='progress'&&!progressInit){{initProgressChart();progressInit=true;}}if(pushHash!==false)history.replaceState(null,null,'#'+t);setTimeout(ivv,100);}}
window.addEventListener('hashchange',function(){{var h=location.hash.slice(1);if(h)switchTab(h,false);}});
(function(){{var h=location.hash.slice(1);if(h)switchTab(h,false);}})();

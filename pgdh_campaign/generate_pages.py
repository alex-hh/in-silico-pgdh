...
.wf-title h2{{font-size:20px;font-weight:700;background:linear-gradient(90deg,#39ff14,#00e5ff,#ff1493);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:3px;text-transform:uppercase}}
...
.wf-g{{color:#39ff14;border-color:#39ff14}}.wf-c{{color:#00e5ff;border-color:#00e5ff}}.wf-p{{color:#ff1493;border-color:#ff1493}}.wf-o{{color:#f0883e;border-color:#f0883e}}.wf-v{{color:#bc8cff;border-color:#bc8cff}}.wf-y{{color:#d29922;border-color:#d29922}}
.wf-glow-g{{filter:drop-shadow(0 0 4px #39ff1480)}}.wf-glow-o{{filter:drop-shadow(0 0 4px #f0883e80)}}.wf-glow-p{{filter:drop-shadow(0 0 4px #ff149380)}}.wf-glow-c{{filter:drop-shadow(0 0 4px #00e5ff80)}}
...
<div class="wf-wrap">
  ...
  <div class="wf-canvas">
    <svg viewBox="0 0 880 520" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M 500 365 Q 470 365 470 395 L 470 430 Q 470 460 430 460 L 380 460" stroke="#ff1493" stroke-width="2" class="wf-glow-p"/>
      <polygon points="382,455 382,465 370,460" fill="#ff1493" class="wf-glow-p"/>
...
function iv(i){{if(init[i])return;var e=document.getElementById('viewer_'+i);if(!e||e.offsetParent===null)return;var d=vd[i];if(!d)return;if(!d.cif){{e.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#8b949e;font-size:14px">3D view available for top {MAX_CIF_EMBEDS} designs</div>';init[i]=true;return;}}var v=$3Dmol.createViewer(e,{{backgroundColor:'#0d1117'}});v.addModel(d.cif,'cif');var bc=d.binderChain,tc2=bc==='A'?'B':'A',c=tclr[d.tool]||'#00CED1';v.setStyle({{chain:tc2}},{{cartoon:{{color:'#ff1493',opacity:0.6}}}});v.setStyle({{chain:bc}},{{cartoon:{{color:c}}}});v.zoomTo();v.render();vs[i]={{viewer:v,binderChain:bc,targetChain:tc2,color:c}};init[i]=true;}}

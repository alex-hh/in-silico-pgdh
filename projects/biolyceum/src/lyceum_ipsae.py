"""Compute ipSAE scores for protein-protein interactions — Lyceum version.
...
def compute_ipsae(pdb_path, pae_file_path, pae_cutoff, dist_cutoff, output_dir):
    """Run the full ipSAE computation and write output files.
    ...
    txt_path = output_dir / f"{path_stem}.txt"
    txt_path.write_text("".join(out_lines))

    byres_path = output_dir / f"{path_stem}_byres.txt"
    byres_path.write_text("".join(out2_lines))

    pml_path = output_dir / f"{path_stem}.pml"
    pml_path.write_text("".join(pml_lines))

    output_files = [txt_path, byres_path, pml_path]
    print(f"\nWrote {len(output_files)} output files:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute ipSAE scores for protein-protein interactions")
    parser.add_argument("--pae-file", required=True, help="Path to PAE file (JSON for AF2/AF3, NPZ for Boltz1)")
    parser.add_argument("--structure-file", required=True, help="Path to structure file (PDB for AF2, CIF for AF3/Boltz1)")
    parser.add_argument("--pae-cutoff", type=float, default=10.0, help="PAE cutoff (default: 10.0)")
    parser.add_argument("--dist-cutoff", type=float, default=10.0, help="Distance cutoff (default: 10.0)")
    parser.add_argument("--output-dir", default="/job/work/output/ipsae", help="Output directory")
    args = parser.parse_args()

    compute_ipsae(
        pdb_path=args.structure_file,
        pae_file_path=args.pae_file,
